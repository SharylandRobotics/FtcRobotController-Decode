...
@Autonomous(name =  "Auto By Encoder (Red)", group = "Robot")
public class AutoByEncoderRed extends LinearOpMode {
    ...
    public void runOpMode(){
        ...
        telemetry.addData("Offset: ", robot.homeToAprilTagRed());
        telemetry.update();
        while (Double.isNaN(robot.homeToAprilTagRed())){
            telemetry.addData("scanning","");
        }
        robot.setTurretPositionRelative(robot.homeToAprilTagRed());
        ...
        robot.setTurretPositionRelative(robot.homeToAprilTagRed() - 5* (robot.homeToAprilTagRed()/Math.abs(robot.homeToAprilTagRed())) );
        sleep(500);
